...
export default function Reports() {
  ...
  const fetchReports = async (pid) => {
    try {
      const res = await api.get(`/reports?patient=${pid || user.id}`);
      setReports(res.data);
    } catch {
  ...
  useEffect(() => {
    if (user.role === 'doctor' && selectedPatient) fetchReports(selectedPatient);
    else if (user.role !== 'doctor') fetchReports();
      ...
      <div className="container-fluid">
        <div className="row mb-4">
          <div className="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
            <h2 className="fw-bold mb-0"><i className="bi bi-file-earmark-medical me-2 text-primary"></i>{user.role === 'doctor' ? 'Patient Reports' : 'Your Reports'}</h2>
            {user.role === 'doctor' && (
        ...
        <div className="row g-4">
          {reports.length === 0 && <div className="col-12 text-center text-muted">No reports found.</div>}
          {reports.map(r => (
